...
function Navbar() {
  ...
  return (
    <>
      <nav
        className={`sticky top-0 z-50 w-full bg-white font-inter transition-shadow duration-300 ${
      ...
      >
        <div className="max-w-7xl mx-auto px-6 py-2 flex justify-between items-center">
          ...
          <div className="flex items-center gap-3">
            <h2 className="text-lg md:text-xl font-bold text-white px-3 py-1 rounded bg-[#0D7332] shadow-md transform transition-transform duration-300 hover:scale-105">
              NCH
            </h2>
          ...
          <div className="hidden md:flex flex-1 justify-center gap-6 font-medium text-gray-700">
            {navItems.map((item, index) => (
              ...
              >
                {item.name}
                <span className="absolute left-0 -bottom-1 w-0 h-1 bg-[#0D7332] transition-all duration-300 group-hover:w-full" />
              </Link>
            ))}
          </div>
          ...
          <div className="md:hidden">
            ...
            >
              {mobileOpen ? (
                <XMarkIcon className="w-6 h-6 transform transition-transform duration-300 rotate-90" />
              ) : (
                <Bars3Icon className="w-6 h-6 transform transition-transform duration-300" />
              )}
            </button>
          </div>
        </div>
            ...
            {navItems.map((item, index) => (
              <Link
                ...
                className={`px-3 py-2 rounded transition-all duration-300 ${
                  location.pathname === item.path
                    ? "text-[#0D7332]"
                    : "hover:bg-gray-100 hover:text-[#0D7332]"
                }`}
              >
                {item.name}
              </Link>
      ...
      {/* Push content down on mobile when menu is open */}
      <div className={`md:hidden transition-all duration-300 ${mobileOpen ? "h-60" : "h-0"}`} />
    </>
  );
}
